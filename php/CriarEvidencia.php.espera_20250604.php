...
register_shutdown_function(function () {
    ...
    if ($error && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        ...
        echo json_encode([
            'sucesso' => false,
            'erro' => 'Erro fatal no servidor (shutdown handler).'
        ]);
        // Flush saída
        if (ob_get_length()) {
            ob_end_flush();
...
if (!is_writable(__DIR__)) {
    error_log("Diretório não é gravável: " . __DIR__);
}
...
$lon = $_POST['longitude'] ?? $_GET['longitude'] ?? '-29.3222'; // fall back é são as coordenadas da ilha de trindade
$lat = $_POST['latitude'] ?? $_GET['latitude'] ?? '-20.5125';
...
file_put_contents($log__file, "SQL: SELECT id_chave_tipo_arquivo FROM tipos_arquivos WHERE extensao = $extensao" . "\n", FILE_APPEND | LOCK_EX);
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($data)) {
            ...
            http_response_code(500); // Set HTTP response code for error
	    file_put_contents($log__file, "\nERRO : erro Arquivos do  tipo ' . $extensao . ' não é suportado", FILE_APPEND | LOCK_EX);
