...
for row in data:
    ...
    print(f"(numeração_do_tipo_de_atividade: {field2}) - Considerando o item de classificação de resultados obtidos por bolsistas de um projeto de economia solidária dado por classificacao_geral: '{field0}', bem como a especificação dos resultados dada por resultado_do_tipo_da_atividade: '{field1}', observando os resquisitos dados por requisitos_do_tipo_da_atividade: '{field3}', crie as frases diferentes que sigam a estrutura de linha CSV separada por | dada por 'token1|tipo_morfologico1|flexao1|token2|tipo_morfologico2|flexao2|token3|tipo_morfologico3|flexao3|token4|tipo_morfologico4|flexao4|token5|tipo_morfologico5|flexao5|numeracao_do_tipo_de_atividade', lembrando que as exceções citadas em requisitos_do_tipo_de_atividade buscam garantir que as frases pertencem a conjuntos de frases disjuntos dois a dois.\n\n", end='')

#    if field0 != previous_field0:
#        print(f"{field0}\n", end='')
#        previous_field0 = field0
#    else:
#        print("\t", end='')  # Mantém o espaçamento se o campo for igual ao anterior
#    
    # Imprime os outros campos com a formatação especificada
#    print(f"\t({field2}) - {field1}\n\tRequisito: {field3}\n\n")
print(f"\n\nquantidade{conta}")
